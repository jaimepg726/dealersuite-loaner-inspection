/**
 * DealerSuite — Loaners Page (Step 53)
 * Shows last inspector and days since last inspection for each loaner.
 */
import { useState, useEffect, useCallback } from 'react'
import { RefreshCw, Car, LogIn, LogOut, Clock, User } from 'lucide-react'
import api from '../../utils/api'
import CheckoutModal from '../../components/dashboard/CheckoutModal'
import CheckinModal from '../../components/dashboard/CheckinModal'

const SC = {
  Out: 'bg-blue-900/50 text-blue-300 border-blue-700',
  Returned: 'bg-green-900/50 text-green-400 border-green-700',
}
    ...
    <div className="card">
      <div className="flex items-center justify-between mb-1">
        <span className="text-lg font-extrabold text-brand-white">
          {loaner.vehicle_display || `Vehicle #${loaner.vehicle_id}`}
        </span>
        <span className={`text-xs font-bold px-2.5 py-1 rounded-full border ${SC[loaner.status] || SC.Out}`}>
          {loaner.status}
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-3 text-xs text-gray-400 mt-1 mb-3">
        ...
        <span className="flex items-center gap-1">
          ...
          {days === 0 ? 'Today' : `${days}d out`}
        </span>
      </div>

      {/* Last inspection info (Step 53) */}
      <div className="flex items-center gap-2 text-xs text-gray-500 mb-3">
        <User className="w-3 h-3 shrink-0" />
        {loaner.last_inspector_name ? (
          <span>
            Last inspected by <span className="text-gray-400">{loaner.last_inspector_name}</span>
            {loaner.days_since_inspection != null && (
              <span className={`ml-1 font-semibold ${loaner.days_since_inspection > 7 ? 'text-yellow-400' : 'text-gray-400'}`}>
                ({loaner.days_since_inspection === 0 ? 'today' : `${loaner.days_since_inspection}d ago`})
              </span>
            )}
          </span>
        ) : (
          <span className="text-gray-600">No inspection on record</span>
        )}
      </div>

      {loaner.status === 'Out' && (
        <button
          onClick={() => onCheckin(loaner)}
  ...
  return (
    <div className="flex flex-col">
          ...
          {['Out', 'Returned', 'All'].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`text-xs font-bold px-3 py-1.5 rounded-full border transition-colors ${
                filter === f ? 'bg-brand-blue border-brand-blue text-white' : 'bg-brand-mid border-brand-accent text-gray-400'
              }`}>
              {f}
            </button>
          ))}
      ...
      <div className="px-5 pb-6 flex flex-col gap-3">
        {loading && [...Array(3)].map((_, i) => <div key={i} className="card h-32 animate-pulse bg-brand-mid" />)}
        ...
        {!loading && !error && !data.length && (
          <div className="flex flex-col items-center gap-3 py-12 text-center">
            ...
            <p className="text-gray-400 font-semibold">
              {filter === 'Out' ? 'No loaners currently out' : 'No loaner records found'}
            </p>
            <p className="text-gray-600 text-sm">Tap Checkout to log a new loaner</p>
          </div>
        )}
        {!loading && data.map(l => (
          <LoanerCard key={l.id} loaner={l} onCheckin={setCheckinTarget} />
        ))}
      </div>

      {showCheckout && (
        <CheckoutModal onClose={() => setShowCheckout(false)} onDone={() => { setShowCheckout(false); load() }} />
      )}
      {checkinTarget && (
        <CheckinModal loaner={checkinTarget} onClose={() => setCheckinTarget(null)} onDone={() => { setCheckinTarget(null); load() }} />
      )}
    </div>
  )
}
